...
fn compose_output(
    ...
    let bpp = layout.bpp;
    // Keep the public `Vec<u8>` fully initialized at every point. The
    // conversion below overwrites the complete logical output on success,
    // but `Vec::set_len` cannot expose uninitialized integer elements even
    // when callers intend to fill them later.
