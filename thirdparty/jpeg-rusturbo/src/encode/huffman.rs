...
impl<W: io::Write> BitWriter<W> {
    ...
    /// Buffer an already byte-aligned entropy segment.
    ///
    /// Restart-row workers finish and byte-stuff their own independent
    /// segments before the caller joins them in raster order. Keeping this
    /// operation on `BitWriter` makes the required aligned state explicit and
    /// lets several ordered segments reach the destination in one write.
    pub(crate) fn write_aligned_entropy(&mut self, bytes: &[u8]) -> io::Result<()> {
        if self.nbits != 0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "cannot append aligned entropy while bits remain pending",
            ));
        }
...
mod tests {
    ...
    #[test]
    fn aligned_entropy_buffers_ordered_segments() {
        let mut out = Vec::new();
        {
            let mut bw = BitWriter::new(&mut out);
            bw.write_aligned_entropy(&[1, 2]).unwrap();
            bw.write_aligned_entropy(&[3, 4]).unwrap();
            bw.flush_to_byte_boundary().unwrap();
        }
    ...
    #[test]
    fn aligned_entropy_rejects_pending_bits() {
        let mut out = Vec::new();
        let mut bw = BitWriter::new(&mut out);
        bw.write_bits(1, 1).unwrap();
        let error = bw.write_aligned_entropy(&[2]).unwrap_err();
