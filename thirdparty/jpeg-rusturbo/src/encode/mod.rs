...
impl<W: Write> JpegEncoder<W> {
    ...
    /// Set the parallelism budget for baseline encoding.
    ///
    ...
    ///
    /// Pixel fetch, color conversion, forward DCT, quantization, and
    /// zigzag work run in parallel. RGBA 4:4:4 scans whose restart interval
    /// is exactly one MCU row also encode independent entropy rows in
    /// parallel. Other scan shapes keep Huffman emission sequential because
    /// their DC predictor chain and bitstream are MCU-ordered. The encoded
    /// JPEG bytes are identical for every thread count.
    ...
    fn encode_inner(
        ...
        let uses_parallel_restart_rows = can_parallelize_restart_rows(
            threads,
            self.subsampling,
            layout,
            width,
...
fn can_parallelize_restart_rows(
    threads: u32,
    subsampling: ChromaSubsampling,
    layout: PixelLayout,
    width: u32,
    ...
    threads != 1
        && matches!(subsampling, ChromaSubsampling::Yuv444)
        && layout == RGBA
        && restart_interval > 0
        && u32::from(restart_interval) == width.div_ceil(Yuv444Scheme::MCU_W)
        && height.div_ceil(Yuv444Scheme::MCU_H) > 1
}
...
fn dispatch_scan<W: Write>(
    ...
    // A restart interval of exactly one MCU row makes every row an
    // independent entropy-coded segment: DC predictors reset at its start,
    // and the preceding row is padded to a byte boundary. Viewr deliberately
    // emits this shape. Encode those rows end-to-end in parallel instead of
    // retaining every quantized coefficient and serializing the dominant
    // Huffman stage afterward.
    if can_parallelize_restart_rows(
        threads,
        subsampling,
        layout,
        width,
        ...
        debug_assert_eq!(u32::from(restart_interval), mcus_x);
        let stripes = run_on_pool(threads, || {
            parallel_encode_restart_stripes::<Yuv444Scheme>(
                pixels, width, height, layout, div_luma, div_chroma, dc_luma, ac_luma, dc_chroma,
                ac_chroma,
            )
        })??;
        return emit_restart_stripes(&stripes, bw);
    }
...
/// Encode contiguous stripes of independent restart rows. One stripe per
/// available worker avoids hundreds of short-lived row allocations while
/// keeping color conversion, DCT, quantization, and Huffman emission parallel.
/// `IndexedParallelIterator::collect` preserves stripe order.
#[allow(clippy::too_many_arguments)]
fn parallel_encode_restart_stripes<S: SamplingScheme>(
    pixels: &[u8],
    width: u32,
    height: u32,
    layout: PixelLayout,
    ...
    let mcus_x = width.div_ceil(S::MCU_W);
    let mcus_y = height.div_ceil(S::MCU_H);
    let stripe_count = (rayon::current_num_threads() as u32).clamp(1, mcus_y);

    // Allocate output ownership on the caller thread. Returning many vectors
    // allocated by worker threads and dropping them on the caller causes
    // cross-thread allocator retention during repeated encodes on macOS.
    let mut stripes: Vec<Vec<u8>> = (0..stripe_count)
        .map(|stripe_index| {
            let start_row = stripe_index * mcus_y / stripe_count;
            let end_row = (stripe_index + 1) * mcus_y / stripe_count;
            let rows = (end_row - start_row) as usize;
            let capacity = (width as usize)
                .saturating_mul(S::MCU_H as usize)
                .saturating_mul(rows)
                .min(pixels.len());
    ...
    stripes.par_iter_mut().enumerate().try_for_each(
        |(stripe_index, stripe)| -> io::Result<()> {
            let stripe_index = stripe_index as u32;
            let start_row = stripe_index * mcus_y / stripe_count;
            let end_row = (stripe_index + 1) * mcus_y / stripe_count;
            let mut stripe_writer = BitWriter::new(stripe);
            // The writer flushes at every row boundary. It needs only one
            // row's scratch capacity, not the whole stripe's capacity.
            stripe_writer.reserve((width as usize).saturating_mul(S::MCU_H as usize));
            for my in start_row..end_row {
                let mut prev_dc = DcPredictors::default();
                for mx in 0..mcus_x {
                    S::encode_one_mcu(
                        &mut stripe_writer,
                ...
                if my + 1 < mcus_y {
                    stripe_writer.write_restart((my as u8) & 7)?;
                } else {
                    stripe_writer.flush_to_byte_boundary()?;
                }
            }
            Ok(())
        },
...
/// Join byte-aligned stripes in raster order. Each stripe already contains
/// the correct markers at its row boundaries, including its final boundary
/// when another stripe follows. The final image row has no trailing marker.
fn emit_restart_stripes<W: Write>(stripes: &[Vec<u8>], bw: &mut BitWriter<W>) -> io::Result<()> {
    let total_bytes = stripes.iter().try_fold(0_usize, |total, stripe| {
        total.checked_add(stripe.len()).ok_or_else(|| {
            io::Error::new(io::ErrorKind::InvalidData, "JPEG entropy length overflow")
        })
    })?;
    bw.reserve(total_bytes);
    for stripe in stripes {
        bw.write_aligned_entropy(stripe)?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn encode_row_restarted_rgba(
        ...
        enc.set_restart_interval(width.div_ceil(Yuv444Scheme::MCU_W) as u16);
        enc.set_threads(threads);
        enc.encode_rgba(pixels, width, height).unwrap();
        drop(enc);
        out
    }

    #[test]
    fn parallel_restart_rows_are_byte_identical_to_serial() {
        #[cfg(miri)]
        let dimensions: &[(u32, u32)] = &[(9, 73)];
        #[cfg(not(miri))]
        let dimensions: &[(u32, u32)] = &[(1, 1), (7, 9), (8, 16), (17, 19), (31, 25), (9, 73)];
        #[cfg(miri)]
        let qualities: &[u8] = &[97];
        #[cfg(not(miri))]
        let qualities: &[u8] = &[1, 50, 97, 100];
        #[cfg(miri)]
        let thread_counts: &[u32] = &[2];
        #[cfg(not(miri))]
        let thread_counts: &[u32] = &[0, 2, 3];
            ...
            for y in 0..height {
                for x in 0..width {
                    pixels.extend_from_slice(&[
                        ...
                        x.wrapping_mul(13).wrapping_add(y.wrapping_mul(19)) as u8,
                    ]);
                }
            }
            for &quality in qualities {
                let serial = encode_row_restarted_rgba(&pixels, width, height, quality, 1);
                for &threads in thread_counts {
                    assert_eq!(
                        encode_row_restarted_rgba(&pixels, width, height, quality, threads),
                        serial,
                        "{width}x{height} q{quality}, threads={threads}",
                    );
                }
            }
        }
